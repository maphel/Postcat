...
| `test/*.test.js` | Unit tests (`node:test`): `lib.test.js` covers `src/lib`, `regressions.test.js` pins fixed bugs, `pack.test.js` packs and checks the zip's contents |
...
| `scripts/pack.js`, `scripts/screenshot.js` | Zip for distribution (excludes `src/build-info.js`; `test/pack.test.js` checks that); renders the README images (`docs/screenshot*.png`: bottom and side docking, light and dark) through the harness |
| `scripts/stamp.js`, `src/build-info.js` | `npm run stamp` writes `src/build-info.js` (generated, git-ignored, never packed: `export default { commit, short, branch, time }`) from `git rev-parse`; `main.js` looks it up with `chrome.runtime.getPackageDirectoryEntry` and imports it when present to extend the version line in the Options menu |
